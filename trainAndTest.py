...
def train(trainArgs):
    ...
    for i in range(trainArgs['epochs']):
        ...
        for batch_idx,(lines, contactmap,properties) in enumerate(tqdm(train_loader)):
            # lines: ('CC(C)(C)[C@H](NC(=O)[C@H](CC1CCCC1)CN(O)C=O)C(=O)c3ccc(N2CCOCC2)c(F)c3',)
            # properties: 0 / 1
        ...
        if(trainArgs['doSave']):
            torch.save(attention_model.state_dict(), './model_pkl/DUDE/'+timeLable()+trainArgs['saveNamePre']+'%d.pkl'%(i+1))
        if(trainArgs['doTest']):
            testArgs = {}
